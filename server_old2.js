...
const EMAIL_FROM = process.env.EMAIL_FROM ||
  `${process.env.EMAIL_FROM_NAME || 'Seeds FinCap Visitor System'} <${process.env.EMAIL_FROM_ADDRESS || 'noreply@seedsfincap.com'}>`;
...
app.post('/api/visitors', async (req, res) => {
  try {
    ...
    try {
      // Build attachments array — QR code always included, photo if captured
      const attachments = [
        {
          filename: `visitor-qr-${visitorId}.png`,
          content: qr.buffer.toString('base64'),
          content_type: 'image/png',
          // Resend uses "inline" disposition with a content_id for CID embedding
          // The HTML references this as src="cid:visitor-qr-code"
          headers: {
            'Content-ID': '<visitor-qr-code>',
            'Content-Disposition': 'inline; filename="visitor-qr.png"'
          }
        }
      ];
      ...
      if (photo_base64) {
        attachments.push({
          filename: `visitor-photo-${visitorId}.jpg`,
          content: photo_base64,
          content_type: 'image/jpeg',
          headers: {
            'Content-ID': '<visitor-photo>',
            'Content-Disposition': 'inline; filename="visitor-photo.jpg"'
          }
        });
      }
